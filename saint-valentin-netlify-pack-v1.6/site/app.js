...
function renderChooser(){
  ...
  ["touchstart","pointerdown","wheel"].forEach(type=>carousel.addEventListener(type,cancelSwipeDemo,{passive:true,once:true}));

  function maybePlaySwipeDemo(){
    if(n < 3 || window.matchMedia?.("(prefers-reduced-motion: reduce)").matches) return;
    let seen = false;
    try { seen = sessionStorage.getItem("sv-swipe-demo-seen") === "1"; } catch(_) {}
    if(seen || window.__svSwipeDemoSeen) return;
    window.__svSwipeDemoSeen = true;
    try { sessionStorage.setItem("sv-swipe-demo-seen","1"); } catch(_) {}
    carousel.classList.add("demoing");
    const go = (delay, virtual) => demoTimers.push(setTimeout(()=>{
      if(demoCancelled) return;
      setActiveFromVirtual(virtual);
      centerVirtual(virtual,"smooth");
    }, delay));
    go(520, n + ((cardIndex + 1) % n));
    go(1450, n + ((cardIndex + 2) % n));
    demoTimers.push(setTimeout(()=>{
      if(!demoCancelled){ carousel.classList.remove("demoing"); normalizeLoop(); }
    },2350));
  }
